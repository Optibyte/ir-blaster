...
class AuthService {
  ...
  static Future<String?> login(String email, String password) async {
    try {
      ...
      debugPrint('🔐 [AuthService] Login Response: ${jsonEncode(mockResponse)}');
      ...
      await _storage.write(key: _companyIdKey, value: mockUser['company'] as String);
      await _storage.write(key: _bucketKey, value: mockUser['bucket'] as String);
  ...
  static Future<Map<String, dynamic>?> verify() async {
    try {
      ...
      if (userDataRaw != null) {
        ...
        debugPrint('✅ [AuthService] Verify Response: {"status": 1, "data": $userDataRaw}');
        return userData;
      }
      return null;
    } catch (e) {
  ...
  static Future<String?> getBucket() async =>
      _storage.read(key: _bucketKey);

  static Future<String?> getSiteId() async =>
      _storage.read(key: _siteIdKey);
